...
const createBookTable = async () => {
  const Book_Table_Query = `CREATE TABLE IF NOT EXISTS shree12_Books(title VARCHAR(100),author VARCHAR(100))`;
  try {
...
const createLoginTable = async () => {
  const Login_Table_Query = `CREATE TABLE IF NOT EXISTS shree12_LoginDetails(id SERIAL PRIMARY KEY, name VARCHAR(50), email VARCHAR(50),username VARCHAR(200),password VARCHAR(2000))`;
  try {
...
app.post("/register", async (req, res) => {
  ...
  try {
    await pool.query(
      "INSERT INTO shree12_LoginDetails(name,email,username,password) VALUES($1, $2, $3, $4)",
...
app.post("/login", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT * FROM shree12_LoginDetails WHERE username = $1",
      [username]
    );
...
app.post("/addBooks", authenticateToken, async (req, res) => {
  try {
    ...
    await pool.query(
      "INSERT INTO shree12_Books (title, author) VALUES ($1, $2)",
      [title, author]
    );
...
app.get("/booksList", authenticateToken, async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM shree12_Books");
...
app.get("/userList", authenticateToken, async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM shree12_LoginDetails");
    const books = result.rows;
    res.json(books);
  } catch (error) {
    console.error(error);
